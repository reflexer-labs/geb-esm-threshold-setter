...
contract ESMThresholdSetterMock {
    // --- Auth ---
    ...
    **/
    ...
    function subtract(uint256 x, uint256 y) internal pure returns (uint256 z) {
        assert((z = x - y) <= x); //, "ESMThresholdSetter/sub-uint-uint-underflow");
    }
    function multiply(uint256 x, uint256 y) internal pure returns (uint256 z) {
        assert(y == 0 || (z = x * y) / y == x); // , "ESMThresholdSetter/multiply-uint-uint-overflow");
    }
    ...
    */
    function modifyParameters(bytes32 parameter, address addr) public isAuthorized {
    ...
    */
    function recomputeThreshold() public virtual {
